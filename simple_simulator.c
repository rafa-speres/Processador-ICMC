...
#define SQR 38      // "100110"; -- SQR Rx 					-- Rx <- Rx^2 	    Format: < inst(6) | Rx(3) | Ry(3) | Rz(3)| x >   MEXI
	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				*/
				case SQR:
					selM3 = rx;			//Seleciona rx nos dois mux de entrada da ULA
					selM4 = rx;			
					OP = MULT;			//Seta a operação da ULA como multiplicação
					carry = 0;			
					selM2 = sULA;		//Seleciona mux2 para a saída da ULA
					LoadReg[rx] = 1;	//Carrega rx com o resultado da operação
					selM6 = sULA;		//Seleciona mux6 para a saída da ULA, possibilitando o LoadFR
					LoadFR  = 1;		
					// -----------------------------
					state=STATE_FETCH;
					break;
